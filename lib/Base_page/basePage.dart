            ...
            children: [
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text("نمایش آگهی های همه شهر ها",style: TextStyle(color:Colors.white),),
                    FittedBox(
                      child: CupertinoSwitch(
                          activeColor: Colors.orangeAccent,
                          trackColor: Colors.grey,
                          value: allCity, onChanged: (res){
                        setState(() {
                          allCity = !allCity;
//                        _bloc.filter[widget.text] = res;
              ...
              Padding(
                padding: const EdgeInsets.symmetric(horizontal:16.0),
                child: Text("فیلتر هزینه",style: TextStyle(color: Colors.white)),
              ),
              RangeSlider(
                  activeColor: Colors.orangeAccent,
              ...
              Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(DigitToWord.toWord(double.parse(minPrice).floor().toString(),StrType.NumWord,isMoney: true),style: TextStyle(color: Colors.white),),
                    Text(DigitToWord.toWord(double.parse(maxPrice).floor().toString(),StrType.NumWord,isMoney: true),style: TextStyle(color:Colors.white),)
                  ],
                ),
              ),
  ...
  Future<bool> handleWillPop(BuildContext context) async {
    ...
    if (backButtonHasNotBeenPressedOrSnackBarHasBeenClosed) {
      ...
      key.currentState.showSnackBar(SnackBar(
        content: Text('برای خروج دوبار کلیک کنید',style: TextStyle(color: Colors.white,fontFamily: "MuliYekan"),),
  ...
  SnackBar snackBar()=> SnackBar(
    content: Text('برای خروج دوبار کلیک کنید',style: TextStyle(color: Colors.white),),
